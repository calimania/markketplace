...
export async function getVisibilityFlags(storeId: string) {
  const [counts, store, magicPages] = await Promise.all([
    getContentCounts(storeId),
  ...
  // Build set of found slugs for fast lookup
  const foundSlugs = new Set(
    magicPages.map((p: any) => p.slug?.toLowerCase()).filter(Boolean)
  );

  // Check which categories have matching pages
  ...
  // Priority: settings > magic pages > content existence
  return {
    show_blog:
